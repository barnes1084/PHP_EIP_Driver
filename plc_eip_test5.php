...
echo "<br><br>0==========================================================================================================================================";
    $sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);  // Create new socket
    if ($sock == false) {
        trigger_error("Error creating socket: " . socket_strerror(socket_last_error()));
    }
	else {echo " <br> Socket created ($sock) ";}
    ...
    $result = socket_connect($sock, $ipaddress, 44818);  // Connect to the PLC using EIP protocol
    if ($result == false) {
        trigger_error("Error connecting to PLC: " . socket_strerror(socket_last_error()));
    }
	else {echo " <br> Socket connected ($result)"; }
...
echo "<br><b><font color=green>function PlcRead(sock, eipSessionHandle, cipConnectionID, tagname) </font></b><br> ";
	...
	echo "<br><b><font color=green><br><br>Sent:  </font></b>"; 
        ...
        $PlcRead_request = "\x70\x00\x34\x00" . $eipSessionHandle . "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\xA1\x00\x04\x00" . $cipConnectionID . "\xB1\x00\x20\x00" . $SequenceNumber_hex .  "\x4C" . $requestPathSize_hex. "\x91" . $tagNameLength_hex . $packedTagName_hex . "\x00\x01\x00";
...
$tagvalue1 = getplctagvalue('10.64.59.28', 1, 0, 'Heartbeat_PLC_To_Server'); echo "<br><b><font color=red> Heartbeat_PLC_To_Server: </font></b>", $tagvalue1[1];
echo "<br>"; echo "<br>";
//$tagvalue2 = getplctagvalue('10.64.59.28', 1, 0, 'Humidity_Low_Limit'); echo "<br><b><font color=red> Humidity_Low_Limit: </font></b>", $tagvalue2[1];
//echo "<br>"; echo "<br>";
